...
if [ "$POWERLINE_RIGHT_B" = "" ]; then
  POWERLINE_RIGHT_B=%D{%I:%M}
elif [ "$POWERLINE_RIGHT_B" = "none" ]; then
  POWERLINE_RIGHT_B=""
fi

if [ "$POWERLINE_RIGHT_A" = "mixed" ]; then
  POWERLINE_RIGHT_A=%(?."$POWERLINE_DATE_FORMAT".%{$fg[red]%}✘ %?)
elif [ "$POWERLINE_RIGHT_A" = "exit-status" ]; then
  POWERLINE_RIGHT_A=%(?.%{$fg[green]%}✔ %?.%{$fg[red]%}✘ %?)
...
if [ "$POWERLINE_GIT_CLEAN" = "" ]; then
  POWERLINE_GIT_CLEAN="%{$fg[green]%}✔"
fi

if [ "$POWERLINE_GIT_DIRTY" = "" ]; then
  POWERLINE_GIT_DIRTY="%{$fg[red]%}✘"
fi
...
if [ "$POWERLINE_GIT_MODIFIED" = "" ]; then
  POWERLINE_GIT_MODIFIED="%F{blue}✹%F{yellow}"
fi
...
PROMPT="%k%f%F{white}%K{blue} "$POWERLINE_CURRENT_PATH"%F{green}"$POWERLINE_GIT_INFO_LEFT" %k"$'\ue0b0'"%f  
▶"
